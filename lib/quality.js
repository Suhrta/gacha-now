...
// ── ブランド一覧ページの品質判定 ─────────────────────────────
// /brand/[slug] は brandSlug ごとに無条件で生成されるため、商品が1〜2件しかない
// ブランドでは「見出し＋数件」だけの薄いハブページになる。
// BRAND_MAP を拡張するほどこの薄いページが増える構造なので、件数で足切りする。
// （商品ページ自体はインデックス対象のまま。ハブだけ検索から外す）
const MIN_BRAND_PRODUCTS = 3;

export function isLowValueBrandPage(brandProducts) {
  return brandProducts.length < MIN_BRAND_PRODUCTS;
}
