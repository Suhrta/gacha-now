...
//
// 表示（components/HubInfo.jsx）と構造化データ（各 layout.jsx の FAQPage）が
...
// extraFaq: そのハブ固有のQ&A（data/series.js の faq）。定義文の直後に差し込む。
// 表記ゆれのように「そのシリーズにしか無い疑問」は共通の定型文では答えられないため。
export function buildHubInfo({ name, items, intro, extraFaq }) {
  ...
  // 定義文は一覧の上（HubLead）、残りの解説は一覧の下（HubDetails）に分けて出す。
  // 同じ intro を上下で二重に見せないよう、切り分けはここでやる。
  const introRest = definition && intro ? intro.slice(definition.length).trim() : intro || null;
  ...
  const faq = [
    ...
    ...(definition
      ...
      : []),
    ...(Array.isArray(extraFaq) ? extraFaq : []),
    {
      q: `${name}の最新ガチャは？`,
      a: [
        {
  ...
  return { stats, faq, definition, introRest, priceText, latestMonth, makers, avgTypes, minPrice, maxPrice };
}
...
// 商品名の先頭に付く販路・形態の但し書きを落とす。
// 例: 【ガシャポンバンダイオフィシャルショップ限定再販】 【フラットガシャポン】
//
// 説明文では22字で切るため、これを残すと商品名が1文字も見えないことがある。
// 実測（2026-09-07）: /series/mejirushi の description が
// 「最新は2026年9月発売の「【ガシャポンバンダイオフィシャルショップ限定…」」で、
// SERPで何の商品か分からない状態だった。全759件中40件が該当し、
// うち17件は但し書きだけで22字を超える。
export function stripLeadingNote(name) {
  if (!name) return name;
  const stripped = name.replace(/^[【〔［[(（][^】〕］\])）]*[】〕］\])）]\s*/, "").trim();
  // 但し書きしか無い商品名は存在しないはずだが、空になったら元の名前を使う
